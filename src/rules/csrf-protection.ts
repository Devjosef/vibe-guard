...
export class CsrfProtectionRule extends BaseRule {
  ...
  private calculateConfidence(baseConfidence: number, context: CsrfContext): number {
    ...
    if (context.hasCsrfProtection) confidence *= 0.6; // Reduces if CSRF protection present
    if (context.hasSecureCookies) confidence *= 0.8; // Reduces if secure cookies present
